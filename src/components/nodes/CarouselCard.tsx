...
const CarouselCard: React.FC<NodeProps> = ({ data, id, isConnectable }) => {
  ...
  const onchangeContent = (event: any, card: RichCardData, type: string, actionID?: string) => {
    if (type === "imageUrl") {
      ...
      if (c.id === card.id && type === "title") {
        ...
        return {
          ...
          buttons: c.buttons.map(button => 
            button.id === actionID ? { ...button, title: event.target.value } : button
          )
        };
      }
      return c;
  ...
  const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>, card: RichCardData) => {
      ...
      <Carousel className="w-full overflow-visible">
        <CarouselContent className='overflow-visible !transform-none'>
          {cards.length > 0 && cards.map((card, index) => (
            <CarouselItem key={card.id} className="!w-[64]">
              <Card className="border rounded-lg shadow-sm relative !w-[64]">
                ...
                <CardHeader className="relative">
                  <Button size="sm" className="!bg-white !text-black border-1 !border-black !p-2 absolute top-6 right-[-30px] -rotate-90" onClick={addCard}>
                  ...
                  <div className="flex items-center bg-white p-1 px-2 rounded text-sm">
                    <span className="mr-1">{index + 1}</span>/<span>{cards.length}</span>
                  </div>
                  <div className="relative h-40 bg-gray-100 flex items-center justify-center">
                  {card.imageUrl ? (
                    <img src={card.imageUrl} alt="Uploaded" className="w-full h-full object-cover" />
                  ) : (
                    <label className="cursor-pointer flex flex-col items-center justify-center w-full h-full relative h-40 bg-gray-100 flex items-center justify-center">
                      ...
                      <input
                        ...
                        accept="image/*"
                        className="hidden"
                        onChange={(e) => onchangeContent(e, card, "imageUrl")}
                      />
                    </label>
                  )}
                  </div>
                ...
                <CardContent className="p-4">
                  ...
                  <div>
                    ...
                    <Textarea 
                      ...
                      onChange={(e) => onchangeContent(e, card, "description")}
                    />
                  </div>
                </CardContent>
                  ...
                  {card.buttons && card.buttons.length > 0 && (
                    <div className="mt-4 w-full">
                      {card.buttons.map((action) => (
                        <div key={action.id} className="relative border border-gray-300 p-3 bg-white">
                          ...
                          <div className="w-full">
                            ...
                            <div className="flex w-full mb-2">
                              <input
                                type="text"
                                value={action.title || ''}
                                onChange={(e) => onchangeContent(e, card, "actionTitle", action.id)}
